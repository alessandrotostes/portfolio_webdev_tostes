...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const projectModalInstance = projectModalEl ? new bootstrap.Modal(projectModalEl) : null;
    ...
    const iframeOverlay = projectModalEl ? projectModalEl.querySelector(".iframe-overlay") : null;
    ...
    if (projectModalEl) {
        projectModalEl.addEventListener("show.bs.modal", function (event) {
            ...
            if (modalProjectTech) modalProjectTech.textContent = tech ? tech.split(",").map(t => t.trim()).join(", ") : "N/A"; // Formata tecnologias

            // Atualiza links
            if (modalProjectLiveLink) {
                modalProjectLiveLink.href = liveUrl && liveUrl !== "#" ? liveUrl : "#";
                modalProjectLiveLink.style.display = liveUrl && liveUrl !== "#" ? "inline-block" : "none";
            }
            if (modalProjectRepoLink) {
                modalProjectRepoLink.href = repoUrl && repoUrl !== "#" ? repoUrl : "#";
                modalProjectRepoLink.style.display = repoUrl && repoUrl !== "#" ? "inline-block" : "none";
            }
            ...
            if (modalProjectIframe && liveUrl && liveUrl !== "#") {
                ...
                modalProjectIframe.onload = () => {
                    ...
                    try {
                        ...
                        console.warn("Iframe loading likely blocked by cross-origin policy for URL:", liveUrl);
                ...
                setTimeout(() => {
                    if (!iframeLoaded) {
                        console.warn("Iframe onload event did not fire. Assuming blocked for URL:", liveUrl);
                        iframeOverlay.style.display = "flex";
                        modalProjectIframe.style.opacity = "0";
                    }
    ...
    document.querySelectorAll("a[href^=\"#\"]").forEach(anchor => {
        anchor.addEventListener("click", function (e) {
            ...
            if (targetElement) {
                ...
                const offsetPosition = elementPosition + window.pageYOffset - navbarHeight - 10; // 10px de margem extra
    ...
    window.addEventListener('scroll', () => {
        ...
        if (currentSectionId === '' && scrollPosition < sections[0]?.offsetTop) {
             document.querySelector('.navbar-nav .nav-link[href="#sobre"]')?.classList.add('active');
        }
    });
